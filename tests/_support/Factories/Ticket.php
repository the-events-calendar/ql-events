...
class Ticket extends \WP_UnitTest_Factory_For_Post {
	...
	public function create_rsvp_ticket( $post_id, array $overrides = [] ) {
		...
		$data = [
				'ticket_name'             => "Test RSVP ticket for " . $post_id,
				'ticket_description'      => "Test RSVP ticket description for " . $post_id,
				'ticket_show_description' => 1,
		];
		$data = array_merge( $data, $overrides );

		return $this->create_ticket( $this->get_rsvp_ticket_provider(), $post_id, 100, $data );
	}
