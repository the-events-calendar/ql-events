...
configure_wordpress() {
    ...
    wp core install --url=wp.test --title="QL Events Tests" --admin_user=admin --admin_password=password --admin_email=admin@wp.test
    wp rewrite structure '/%year%/%monthnum%/%postname%/'
}

setup_the_events_calendar() {
	echo "Installing & Activating The Events Calendar"
	wp plugin install the-events-calendar --activate

	echo "Installing & Activating Event Tickets"
...
setup_plugin() {
	...
	if [ ! -d $WP_CORE_DIR/wp-content/plugins/ql-events ]; then
		ln -s $PLUGIN_DIR $WP_CORE_DIR/wp-content/plugins/ql-events
	fi
