...
configure_wordpress() {
    ...
    wp core install --url=$WP_DOMAIN --title="WPGraphQL TEC Tests" --admin_user=$ADMIN_USERNAME --admin_password=$ADMIN_PASSWORD --admin_email=$ADMIN_EMAIL
...
setup_plugin() {
	...
	if [ ! -d $WP_CORE_DIR/wp-content/plugins/wp-graphql-tec ]; then
		ln -s $PLUGIN_DIR $WP_CORE_DIR/wp-content/plugins/wp-graphql-tec
