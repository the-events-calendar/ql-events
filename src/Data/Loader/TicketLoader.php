...
use WPGraphQL\TEC\Model\RsvpTicket;
use WPGraphQL\TEC\Model\Ticket;
use WPGraphQL\TEC\Model\PurchasableTicket;
...
class TicketLoader extends AbstractDataLoader {
	...
	protected function get_model( $entry, $key ) : ?Ticket {
		if (
			...
			! in_array( $entry->post_type, [ ...array_keys( Utils::get_et_types() ), 'revision' ], true )
		) {
			return null;
		}
