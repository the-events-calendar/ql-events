<?php
/**
 * DataLoader - OrganizerLoader
 *
 * Loads Models for TEC Organizer post type.
 *
 ...
 */
class OrganizerLoader extends AbstractDataLoader {
	/**
	 * {@inheritDoc}
	 */
	protected function get_model( $entry, $key ) : ?Organizer {
		if (
			empty( $entry ) ||
		...
		if ( 'revision' === $entry->post_type && ! empty( $entry->post_parent ) && absint( $entry->post_parent ) ) {
			...
			$context->get_loader( 'tribe_organizer' )->load_deferred( $post_parent );
		}

		$post = new Organizer( $entry );
	...
	public function loadKeys( array $keys ) {
		...
		foreach ( $keys as $key ) {
			...
			if ( 'tribe_organizer' !== $post_object->post_type ) {
				/* translators: invalid post-type error message */
				throw new UserError( sprintf( __( '%s is not a valid Organizer post type', 'wp-graphql-tec' ), $post_object->post_type ) );
			}
