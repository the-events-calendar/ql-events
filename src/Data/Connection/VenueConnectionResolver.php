<?php
/**
 * Resolves connections to Venues
 *
 ...
 */
class VenueConnectionResolver extends PostObjectConnectionResolver {
	...
	public function should_execute() {
		...
		if ( isset( $this->post_type ) && 'revision' === $this->post_type ) {
			if ( $this->source instanceof Venue ) {
				...
				if ( ! current_user_can( 'edit_tribe_venue' ) ) {
					$this->should_execute = false;
				}
			}
		}
	...
	public function get_query_args() {
		...
		return apply_filters(
			'graphql_tec_venue_connection_query_args',
			$query_args,
			$this->source,
			$this->args,
			$this->context,
	...
	public function sanitize_input_fields( array $where_args ) {
		 ...
		 * @since 0.0.5
		 */
		$query_args = apply_filters( 'graphql_map_input_fields_to_venue_query', $query_args, $where_args, $this->source, $this->args, $this->context, $this->info, $this->post_type );
